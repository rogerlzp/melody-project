...
/**
 * Created by liuyw on 2015/11/16.
 */
...
public class RedisCache {
    ...
    // 30 天超时 TODO ConfigurationManager
    public static int expireMsecs30day = 60 * 60 * 24 * 30;
    // 默认一天超时 TODO ConfigurationManager
    public static int expireMsecsToday = 60 * 60 * 24 * 15;

    /**
     * 执行器，{@link RedisCache}的辅助类， 它保证在执行操作之后释放数据源returnResource(jedis)
     *
     * @param <T>
     * @author fengjc
     * @version V1.0
     ...
     *
     * @param key    key值
     * @param expire unix时间戳，从1970-01-01 00:00:00开始到现在的秒数
     * @return 1：设置了过期时间 0：没有设置过期时间/不能设置过期时间
     */
    public Long expireAt(final String key, final long expire) {
        return new Executor<Long>(shardedJedisPool) {

            @Override
            Long execute() {
                return jedis.expireAt(key, expire);
            }
        }.getResult();
    }
    ...
    public boolean setObject(final String key, Object value) {
        final String objectJson = JSON.toJSONString(value);
     ...
     * @return
     */
    public boolean setObject(final String key, Object value, final int expire) {
    ...
    public <T> T getObject(final String key, Class<T> clazz) {
        String value = new Executor<String>(shardedJedisPool) {
            @Override
            String execute() {
                return jedis.get(key);
            }
     ...
     *
     * @param key   key
     * @param field field-value的map
     * @param value 值
     * @return 如果命令执行成功，返回 OK 。当 key 不是哈希表(hash)类型时，返回一个错误。
     */
    public Long hashMultipleSet(final String key, final String field, final String value) {
        return new Executor<Long>(shardedJedisPool) {
    ...
    public String setSeeionKey(final String userId, final String mobileNo) {
        return new Executor<String>(shardedJedisPool) {
            @Override
            String execute() {// mobileNo
                String token = TokenUtils.getInstance().generateToken(mobileNo);
                jedis.setex(RedisCodes.SESSION + token, expireMsecsToday, mobileNo);
                return token;
            }
        }.getResult();
    }
