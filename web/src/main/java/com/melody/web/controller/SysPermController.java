...
//    @Autowired
    ...
    public Json listAllPermission() {
        ...
        List<SysPerm> list = sysPermService.selectList(params);
        if (list.isEmpty()){
    ...
    public Json listButtonPermMapGroupByParent() {
        ...
        List<SysPerm> buttonPermList = sysPermService.selectList(params);
    ...
    public Json syncMenuPermission(@RequestBody String body) {
        ...
        if (!notSyncedPerms.isEmpty()){
            sysPermService.delete(new EntityWrapper<SysPerm>().eq("ptype",PermType.MENU));
            sysPermService.saveOrUpdate(notSyncedPerms);
        }
        return Json.succ(oper);
    }
    ...
    public Json syncApiPermission(@RequestBody String body) {
        ...
        List<SysPerm> notSyncedPerms = JSON.parseArray(body, SysPerm.class);
        if (!notSyncedPerms.isEmpty()){
            sysPermService.delete(new EntityWrapper<SysPerm>().eq("ptype",PermType.API));
            sysPermService.saveOrUpdate(notSyncedPerms);
        }
        return Json.succ(oper);
    }
    ...
    public Json add(@RequestBody String body) {
        ...
        SysPerm permDB = sysPermService.selectOne(params);
        ...
        boolean success = sysPermService.insert(perm);
    ...
    public Json delete(@RequestBody String body) {
        ...
        boolean success = sysPermService.deleteById(pval);
        return Json.result(oper, success);
    }
    ...
    public Json update(@RequestBody String body) {
        ...
        boolean success = sysPermService.updateById(updateData);
